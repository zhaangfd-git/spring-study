...
import com.zhangfd.spring.beans.AbstractPropertyAccessor;
...
import com.zhangfd.spring.factory.support.PropertyAccessorUtils;
...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	private void processKeyedProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		...
		if (propValue.getClass().isArray()) {
			...
			try {
				convertedValue = convertIfNecessary(tokens.canonicalName, oldValue, pv.getValue(),
			...
			try {
				convertedMapKey = convertIfNecessary(null, null, lastKey, mapKeyType, typeDescriptor);
			} catch (Exception e) {
				e.printStackTrace();
			}
			Object oldValue = null;
			...
			try {
				convertedMapValue = convertIfNecessary(tokens.canonicalName, oldValue, pv.getValue(),
...
//		}
				...
				for (int i = 0; i < tokens.keys.length; i++) {
					...
					if (value == null) {
						...
						*/}
						...
						Object convertedMapKey = null;
						try {
							convertedMapKey = convertIfNecessary(null, null, key, mapKeyType, typeDescriptor);
						} catch (Exception e) {
							e.printStackTrace();
						}
	 ...
	 */
	protected abstract BeansException createNotWritablePropertyException(String propertyName);
