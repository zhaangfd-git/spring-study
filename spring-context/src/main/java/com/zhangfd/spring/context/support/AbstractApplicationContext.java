...
import com.zhangfd.spring.context.event.*;
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//执行BeanFactoryPostProcessor接口的实现类方法
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				//收集所有的BeanPostProcessor实现类，注册到beanFactory里
				...
				//创建SimpleApplicationEventMulticaster对象到spring容器
	...
	protected void prepareRefresh() {
		...
		//初始化ApplicationListener集合对象
		if (this.earlyApplicationListeners == null) {
			this.earlyApplicationListeners = new LinkedHashSet<>(this.applicationListeners);
		}
		else {
			// Reset local application listeners to pre-refresh state.
			//若ApplicationListeners集合不为空，则先清空
			this.applicationListeners.clear();
			//把预设的ApplicationListene对象集合放入该集合
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		refreshBeanFactory();//给这个容器起一个唯一别名：赋值唯一id
		//交给子类创建beanFactory = new DefaultListableBeanFactory()对象，这样
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//如果一个实例是EnvironmentAware或EmbeddedValueResolverAware或ResourceLoaderAware或ApplicationEventPublisherAware
		//或MessageSourceAware或ApplicationContextAware的实现类，这在这个类初始化前调用对应接口实现方法的具体方法
		...
		//定义一个实例类，若属性定义了BeanFactory类型的且使用了注解@Autowiring，返回的是上面创建的DefaultListableBeanFactory类的对象.
		// 属性定义了ResourceLoader、ApplicationEventPublisher、ApplicationContext类型的，则返回唯一的一个AbstractApplicationContext实现类对象
		//spring对此做了特殊的处理，具体调用DefaultListableBeanFactory#doResolveDependency方法里的DefaultListableBeanFactory#findAutowireCandidates
		//代码里体现这个逻辑
		...
		//这里赋值一个BeanPostProcessor接口的实现类：ApplicationListenerDetector，它的功能是啥呢？
		//比如我们定义一个ApplicationListener的是是实现类，并且交给spring容器去创建对象，他们在这个对象初始化的后期
		//侦测到这个是ApplicationListener接口的实现类，它就会被赋值到AbstractApplicationContext的applicationListeners属性里，以便后边使用
	 ...
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 * 把beanName=applicationEventMulticaster的SimpleApplicationEventMulticaster对象注入beanFactory
	 * 若容器已经存在beanName=applicationEventMulticaster的对象，则使用用户自定义的ApplicationEventMulticaster实现类，
	 * 一般不建议自己定义ApplicationEventMulticaster的实现类
	 */
